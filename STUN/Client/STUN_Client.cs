...
namespace LumiSoft.Net.STUN.Client
{
    ...
    public class STUN_Client
    {
        ...
        {
            var ip = System.Net.Dns.GetHostAddresses(host)[0];
            return Query(ip, port, socket);
        }
        ...
        /// <returns>Returns UDP netwrok info.</returns>
        /// <exception cref="Exception">Throws exception if unexpected error happens.</exception>
        public static STUN_Result Query(IPAddress ip,int port,Socket socket)
        {
            if(ip == null){
                throw new ArgumentNullException("host");
            }
            ...
            var publicEndPoints = new List<IPEndPoint>();
            ...
            */
            ...
            try{
                ...
                if(test1response == null){
                    ...
                    publicEndPoints.Add(test1response.MappedAddress);
                    ...
                    if(socket.LocalEndPoint.Equals(test1response.MappedAddress)){
                        ...
                        {
                            if (!test2Response.MappedAddress.Equals(test1response.MappedAddress))
                                publicEndPoints.Add(test2Response.MappedAddress);

                            return new STUN_Result(STUN_NetType.OpenInternet, publicEndPoints);
                        }
                        // Symmetric UDP firewall.
                        else{
                            return new STUN_Result(STUN_NetType.SymmetricUdpFirewall, publicEndPoints);
                        }
                    }
                    // NAT
                    else{
                        ...
                        if(test2Response != null)
                        {
                            ...
                            return new STUN_Result(STUN_NetType.FullCone, publicEndPoints);
                        }
                        else{
                            /*
                            ...
                            */
                            ...
                            if(test12Response == null){
                                ...
                                {
                                    publicEndPoints.Add(test12Response.MappedAddress);
                                    return new STUN_Result(STUN_NetType.Symmetric, publicEndPoints);
                                }
                                else{
                                    // Test III
                                    ...
                                    {
                                        if (!test3Response.MappedAddress.Equals(test1response.MappedAddress))
                                            publicEndPoints.Add(test3Response.MappedAddress);

                                        return new STUN_Result(STUN_NetType.RestrictedCone, publicEndPoints);
                                    }
                                    // Port restricted
                                    else{
                                        return new STUN_Result(STUN_NetType.PortRestrictedCone, publicEndPoints);
                                    }
                                }
                            }
                        }
        ...
        public static IPAddress GetPublicIP(string stunServer,int port,IPAddress localIP)
        {
            ...
            if(result.PublicEndPoints != null){
                return result.PublicEndPoints[0].Address;
            }
            else{
